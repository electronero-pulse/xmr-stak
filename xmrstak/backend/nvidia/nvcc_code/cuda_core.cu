		...
		uint32_t startNonce, uint32_t * __restrict__ d_input )
{
	...
	for(int i = start; i < end; ++i)
	{
		...
		if(ALGO == cryptonight_monero_v8 && sub == 1 || ALGO == cryptonight_pulse && sub == 1 || ALGO == cryptonight_electronero && sub == 1)
		{
	...
	for ( i = start; i < end; ++i )
	{
		...
		for ( int x = 0; x < 2; ++x )
		{
			...
			if(ALGO == cryptonight_monero || ALGO == cryptonight_aeon || ALGO == cryptonight_ipbc || ALGO == cryptonight_stellite || ALGO == cryptonight_masari || ALGO == cryptonight_bittube2 || ALGO == cryptonight_electronero || ALGO == cryptonight_pulse)
			{
				...
				{ //Remove CryptonightPulse/CryptonightElectronero after hard fork 17
...
void cryptonight_core_gpu_hash(nvid_ctx* ctx, uint32_t nonce)
{
	...
	if(ALGO == cryptonight_pulse || ALGO == cryptonight_electronero)
	{
		// cryptonight_pulse used 4 full rounds over the scratchpad memory
		roundsPhase3 *= 4;
	}
...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, xmrstak_algo miner_algo, uint32_t startNonce)
{
	...
	static const cuda_hash_fn func_table[] = {
		...
		cryptonight_core_gpu_hash<CRYPTONIGHT_ELECTRONERO_ITER, CRYPTONIGHT_MASK, CRYPTONIGHT_ELECTRONERO_MEMORY/4, cryptonight_electronero, 0>,
		cryptonight_core_gpu_hash<CRYPTONIGHT_ELECTRONERO_ITER, CRYPTONIGHT_MASK, CRYPTONIGHT_ELECTRONERO_MEMORY/4, cryptonight_electronero, 1>,
		
		cryptonight_core_gpu_hash<CRYPTONIGHT_PULSE_ITER, CRYPTONIGHT_MASK, CRYPTONIGHT_PULSE_MEMORY/4, cryptonight_pulse, 0>,
		cryptonight_core_gpu_hash<CRYPTONIGHT_PULSE_ITER, CRYPTONIGHT_MASK, CRYPTONIGHT_PULSE_MEMORY/4, cryptonight_pulse, 1>
	};
