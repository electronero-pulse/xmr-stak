...
extern "C" int cryptonight_extra_cpu_init(nvid_ctx* ctx)
{
	...
	)
	{
		...
		CUDA_CHECK(ctx->device_id, cudaMalloc(&ctx->d_ctx_state2, 50 * sizeof(uint32_t) * wsize));
			...
			cryptonight_monero_v8 == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgoRoot() || cryptonight_pulse == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgo() ||
			cryptonight_pulse == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgoRoot() || cryptonight_electronero == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgo() ||
			cryptonight_electronero == ::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgoRoot())
	{
...
extern "C" void cryptonight_extra_cpu_prepare(nvid_ctx* ctx, uint32_t startNonce, xmrstak_algo miner_algo)
{
	...
	if(miner_algo == cryptonight_pulse)
	{
		CUDA_CHECK_KERNEL(ctx->device_id, cryptonight_extra_gpu_prepare<cryptonight_pulse><<<grid, block >>>( wsize, ctx->d_input, ctx->inputlen, startNonce,
			ctx->d_ctx_state,ctx->d_ctx_state2, ctx->d_ctx_a, ctx->d_ctx_b, ctx->d_ctx_key1, ctx->d_ctx_key2 ));
	}
	if(miner_algo == cryptonight_electronero)
	{
		CUDA_CHECK_KERNEL(ctx->device_id, cryptonight_extra_gpu_prepare<cryptonight_electronero><<<grid, block >>>( wsize, ctx->d_input, ctx->inputlen, startNonce,
			ctx->d_ctx_state,ctx->d_ctx_state2, ctx->d_ctx_a, ctx->d_ctx_b, ctx->d_ctx_key1, ctx->d_ctx_key2 ));
	}
	else
	{
...
#undef XMRSTAK_PP_TOSTRING1
	...
	if(ctx->device_threads == -1)
	{
		...
		bool useCryptonight_Pulse = 
			::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgo() == cryptonight_pulse ||
			::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgoRoot() == cryptonight_pulse;
		
		bool useCryptonight_Electronero = 
			::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgo() == cryptonight_electronero ||
			::jconf::inst()->GetCurrentCoinSelection().GetDescription(1).GetMiningAlgoRoot() == cryptonight_electronero

		// overwrite default config if cryptonight_monero_v8 is mined and GPU has at least compute capability 5.0
		if(useCryptonight_v8 && gpuArch >= 50 || useCryptonight_Electronero && gpuArch >= 50 || useCryptonight_Pulse && gpuArch >= 50)
		{
